...
public:
    ...
    Foo(string n) : name(n) {}
    ~Foo() { cout << "  --" << name << " was destroyed" << endl; }
};
...
public:
    ...
    Bar(string n) : name(n) {}
    ~Bar() { cout << "  --" << name << " was destroyed" << endl; }
};
...
shared_ptr<Person> sharedPointerOne() {
    ...
    cout << "Returning the shared pointer... won't get deleted until end of main" << endl;
    return sp1;
}
...
int main()
{
    ...
    //-------------------------------
    cout << "Making a cycle of Foo objects using shared_ptr's. Notice no deletions..." << endl;
    sharedPointerTwo();

    //-------------------------------
    cout << "Making a cycle of Bar objects using weak_ptr's. They get cleaned up..." << endl;
    weakPointerOne();

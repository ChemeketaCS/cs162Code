...
public:
    ...
    ////--------------------------------------------------------------
    ///     New Features... Assignment, subscript and stream insertion
...
void ArrayList<T>::insertAt(int location, const T& insertItem)
{
    if(location < 0 || location > currentSize) {
        stringstream s;
        s << "Can't insert at index " << location;
        throw out_of_range(s.str());
    }
...
template<class T>
ostream& operator<<(ostream& stream, ArrayList<T>& theList) {
    cout << "(";
    //print all but last

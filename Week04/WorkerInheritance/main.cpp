...
void printSalaried(Employee* employeeList[], int size) {
    for(int i = 0; i < size; i++) {
        ...
        Employee* currentEmployeePtr = employeeList[i];
        //see if it can be made into a pointer to a salaried (this would include salespersons)
        SalariedEmployee* currentAsSalaried = dynamic_cast<SalariedEmployee*>( currentEmployeePtr );
...
int main()
{
    ...
    //An array of Employees would slice off all the subclass information for each object

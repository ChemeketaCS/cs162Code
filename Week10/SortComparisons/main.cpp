...
void insertionSort(vector<T>& vec) {
    //There is no standard insertion sort, but we can implement it using std functions:
    for(auto it = vec.begin(); it != vec.end(); it++){
        ...
        // Shifts the current value to that location and everything else to the right
...
int main()
{
    ...
    for(int size = 1000; size < 100000000; size *= 2) {
        ...
        vector<int> v(size);
        for(int& i : v) { i = rand() * 10000 + rand(); } //fill with random values

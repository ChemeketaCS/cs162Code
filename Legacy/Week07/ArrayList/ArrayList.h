...
T ArrayList<T>::get(int location) const {
  if (location < 0 || location >= currentSize) {
    string msg = "Bad index in get: " + to_string(location) + " with size "
...
void ArrayList<T>::set(int location, const T& repItem) {
  if (location < 0 || location >= currentSize) {
    string msg = "Bad index in set: " + to_string(location) + " with size "
                 + to_string(currentSize);
    throw out_of_range(msg);
  }
...
void ArrayList<T>::removeAt(int location) {
  if (location < 0 || location >= currentSize) {
    string msg = "Bad index in removeAt: " + to_string(location) + " with size "
                 + to_string(currentSize);
    throw out_of_range(msg);
  }
...
void ArrayList<T>::insertAt(int location, const T& insertItem) {
  if (location < 0 || location > currentSize) {
    string msg = "Bad index in insertAt: " + to_string(location) + " with size "
                 + to_string(currentSize);
    throw out_of_range(msg);
  }

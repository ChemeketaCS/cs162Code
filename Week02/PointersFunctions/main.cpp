...
//Take two memory adderesses as parameters - can change what is at those addresses
...
//Take references to two memory addresses - we can change the addresses themselves
...
int main() {
    ...
    //The value pointer1 points to has almost certainly changed while doing last cout
    ...
    int* p1 = &num1;
    int* p2 = &num2;
    cout << "p1 and p2 start as " << p1 << "(value there is " << *p1 << ") "
                                  << p2 << "(value there is " << *p2 << ")" << endl;

    cout << "Swaping the contents of memory they point to:" << endl;
    swap3(p1, p2);
    cout << "After swap3        " << p1 << "(value there is " << *p1 << ") "
                                  << p2 << "(value there is " << *p2 << ")" << endl;

    cout << "Swaping the addresses they point to:" << endl;
    swap4(p1, p2);
    cout << "After swap4        " << p1 << "(value there is " << *p1 << ") "
                                  << p2 << "(value there is " << *p2 << ")" << endl;
